...
#   ./install.sh gemini -- second-opinion     # Specific skill to Gemini CLI
#   ./install.sh claude cursor opencode       # All skills to multiple platforms
#   ./install.sh all                          # All skills to all platforms
#   ./install.sh all -- slide-generator       # Specific skill to all platforms
...
install_skill() {
    ...
    if ! is_skill_dir "$skill_src"; then
...
main() {
    ...
    for p in "${platforms[@]}"; do
        ...
        # Skip if target is the source directory
        if [[ "$target_dir" == "$SCRIPT_DIR" ]]; then
            warn "Skipping $p - target is the source directory"
            continue
        fi
    ...
    for p in "${platforms[@]}"; do
        if [[ "$p" == "claude" ]]; then
            local target_dir
            target_dir=$(get_target_dir claude)
            if [[ "$target_dir" != "$SCRIPT_DIR" ]]; then
